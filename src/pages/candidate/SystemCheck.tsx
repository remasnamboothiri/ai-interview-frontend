...
export const SystemCheck = () => {
  ...
  // ✅ Check interview status FIRST — redirect if already completed
  useEffect(() => {
    if (!uuid) return;
  ...
  // Don't render until status is checked
  if (!statusChecked) {
    return (
        ...
        <CardContent className="p-8">
          ...
          {allChecked && (
            <div className="bg-green-50 border border-green-200 rounded-lg p-6 mb-8">
              ...
              <p className="text-sm text-green-700">Your device and connection are ready for the interview.</p>
            </div>
          )}
          <div className="flex gap-4">
            <Link to={`/interview/waiting-room/${uuid}`} className="flex-1">
              <Button variant="primary" className="w-full" disabled={!allChecked}>Continue to Waiting Room</Button>
            </Link>
            <Button variant="outline" className="flex-1">Run Check Again</Button>
          </div>
        </CardContent>
      </Card>
    </div>
