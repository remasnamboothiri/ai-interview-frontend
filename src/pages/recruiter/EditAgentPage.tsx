...
interface CustomQuestion {
  id: string;
  dbId?: number; // Track existing DB questions for update/delete
  question: string;
  category: string;
...
export const EditAgentPage = () => {
  ...
  const loadAgentData = async () => {
    ...
    try {
      ...
      try {
        const questions = await agentService.getAgentQuestions(parseInt(id));
        if (questions && questions.length > 0) {
          setCustomQuestions(questions.map((q: any, index: number) => ({
            id: `existing-${q.id || index}`,
            dbId: q.id,
          ...
          // No existing questions — show one empty field
          setCustomQuestions([{
            id: '1',
            question: '',
            category: 'technical',
  ...
  const updateQuestion = (questionId: string, field: keyof CustomQuestion, value: string | number) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const agentData: any = {
        ...
        // Include questions — backend will replace all existing questions
    ...
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <CardContent className="space-y-4">
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                ...
                <Input required placeholder="e.g., Senior Technical Interviewer" value={formData.name}
                  onChange={(e) => setFormData({ ...formData, name: e.target.value })} />
              </div>
              <div>
                <label className="block text-sm font-medium text-secondary mb-2">Interview Type *</label>
            ...
            <div>
              ...
              <Textarea required rows={3} placeholder="Describe the agent's purpose..."
            ...
            <div>
              ...
              <Textarea required rows={5} placeholder="Define the agent's personality..."
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-secondary mb-2">Voice Settings *</label>
                <Select value={formData.voice_settings}
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-3">
              ...
              {isAdmin && (
                <label className="flex items-center p-4 border-2 rounded-xl cursor-pointer hover:bg-primary-50 transition-colors"
                  style={{ borderColor: formData.agent_type === 'global' ? 'rgb(22, 163, 74)' : 'rgb(229, 231, 235)' }}>
                  <input type="radio" name="agentType" value="global"
                  ...
                  {formData.agent_type === 'global' && <Badge variant="success">Selected</Badge>}
                </label>
              )}

              {/* Private option — visible to both */}
              <label className="flex items-center p-4 border-2 rounded-xl cursor-pointer hover:bg-primary-50 transition-colors"
                style={{ borderColor: formData.agent_type === 'private' ? 'rgb(22, 163, 74)' : 'rgb(229, 231, 235)' }}>
                <input type="radio" name="agentType" value="private"
                ...
                {formData.agent_type === 'private' && <Badge variant="success">Selected</Badge>}
              </label>

              {!isAdmin && (
                <p className="text-xs text-neutral-500 italic">
                  Recruiters can only manage private agents for their company.
                </p>
              )}
            </div>
            ...
            {formData.agent_type === 'private' && (
              <div>
                <label className="block text-sm font-medium text-secondary mb-2">Select Company *</label>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <Input type="number" min="500" max="4000" step="100" value={formData.max_tokens}
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <Badge variant={totalWeight === 100 ? 'success' : 'warning'}>Total: {totalWeight}%</Badge>
                <Button type="button" size="sm" onClick={addCriteria}>
                  <Plus className="w-4 h-4 mr-2" /> Add Criteria
                </Button>
              </div>
            </div>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            {evaluationCriteria.map((criteria, index) => (
              <div key={criteria.id} className="flex items-center gap-3 p-3 border-2 border-neutral-200 rounded-lg">
                ...
                <div className="flex-1">
                  <Input required placeholder="e.g., Technical Knowledge" value={criteria.name}
                    onChange={(e) => updateCriteria(criteria.id, 'name', e.target.value)} />
                </div>
                <div className="w-32">
                  <div className="flex items-center gap-2">
                    <Input type="number" min="0" max="100" required placeholder="0" value={criteria.weight}
                      onChange={(e) => updateCriteria(criteria.id, 'weight', parseInt(e.target.value) || 0)} />
                    <span className="text-sm text-neutral-600">%</span>
                  </div>
                </div>
                {evaluationCriteria.length > 1 && (
                  <Button type="button" size="sm" variant="ghost" onClick={() => removeCriteria(criteria.id)}>
            ...
            {totalWeight !== 100 && (
              <div className="p-3 bg-warning-50 border border-warning-200 rounded-lg">
                <p className="text-sm text-warning-800">Weights must total 100%. Current total: {totalWeight}%</p>
              </div>
            )}
          </CardContent>
        </Card>

        {/* ── Default Interview Questions (NOW LOADS EXISTING + EDIT/REMOVE/ADD) ── */}
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
          ...
          <CardContent className="space-y-4">
            <p className="text-sm text-neutral-600">
              Edit, remove, or add interview questions. Changes are saved when you update the agent.
            </p>

            {customQuestions.length === 0 && (
              <div className="text-center py-6 text-neutral-500">
                <p className="text-sm">No questions configured. Click "Add Question" to add one.</p>
              </div>
            )}
              ...
              <div key={question.id} className="p-4 border-2 border-neutral-200 rounded-xl space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    {question.dbId && (
                      <Badge variant="neutral" className="text-[10px]">Existing</Badge>
                    )}
                  </div>
                ...
                <div>
                  <label className="block text-sm font-medium text-secondary mb-2">Question Text *</label>
                  <Textarea
                    required
                    rows={2}
        ...
        <div className="flex items-center justify-end gap-3">
          <Button type="button" variant="outline" onClick={() => navigate(ROUTES.AI_AGENTS)}>Cancel</Button>
