...
import { UserPlus, Save, X, Upload, Plus, GraduationCap, FileText } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!formData.resume_file) {
        setError('Please upload the candidate\'s Resume / CV before submitting.');
        setIsSaving(false);
        return;
      }
      ...
      const newUser = await adminService.createUser(userData as any);
      // Step 2: Create candidate profile linked to user
      const candidateData = {
        user_id: newUser.id,
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <div 
                ...
                className="border-2 border-dashed border-neutral-300 rounded-xl p-6 text-center hover:border-primary-400 transition-colors cursor-pointer"
              >
                {formData.resume_file ? (
                  <div className="flex items-center justify-center gap-2">
                    <FileText className="w-8 h-8 text-primary-600" />
                    <div>
                      <p className="text-sm font-medium text-secondary">{formData.resume_file.name}</p>
                      <p className="text-xs text-neutral-500">{(formData.resume_file.size / 1024).toFixed(2)} KB</p>
                    </div>
                  </div>
                ) : (
                  <>
